...
public class AwsS3Service {
    ...
    // 메소드 선언부에 예외 처리 던지기 구성 x, 자동으로 처리해줌
    @SneakyThrows
    public String upload(MultipartFile file) {
        // 파일이 비워잇는지, 이름이 널인지등등 검사
        if(file.isEmpty() || Objects.isNull(file.getOriginalFilename())) {
            throw new Exception("파일 누락 혹은 비워있음");
        }
        // 업로드한 파일을 이미지로 제한한다면등 -> 형식 제한 (JS에서 진행, 백엔드 처리가능함)
        return checkAndUpload( file );
    }
    private String checkAndUpload(MultipartFile file) {
        // check -> 확장자 검사, 파일의 헤더 검사(매직코드등등) 유효성 판단할수 있다 -> 보류
        // s3 upload
        // 업로드시
        // - 파일명 고려(원본유지->중복 발생 가능성 존재, 고유한 이름 필요(UUID 값 추가) => 이름변조
        // - or s3 내에 계정별로 디렉토리를 구성하여 업로드(이름변조 추가) -> 비즈니스 고려
        // 1. 원본 파일명 획득
        String originalFilename = file.getOriginalFilename();
        System.out.println("originalFilename : " + originalFilename);

        // 2. .을 중심으로 이름과 확장자 획득 가능 -> 이름변조후 결합 or image/png|jpge... 마임타입(mime)
        ...
        String url="";
        try {
            // 4. 스트림 구성(자바는 스트림을 통해서 통신 진행) -> 오픈
            ...
            // 5. (핵심)s3 업로드 수행 (API 다양하게 제공되고 있음)
